...
/**
 * 前台服务：整条管线的宿主。
 * AudioPlaybackCapture 内录 → PcmProcessor → GeminiLiveClient → SubtitleOverlay / TranscriptLogger
 */
class CaptureService : Service() {
    ...
    private fun start(intent: Intent) {
        ...
        val record = createAudioRecord(proj)
        if (record == null) {
            Log.e(TAG, "AudioRecord init failed")
            StatusBus.connState = "error:内录初始化失败"
            stopEverything()
            return
        }
        audioRecord = record
        ...
        Log.i(TAG, "pipeline started: rate=${record.sampleRate} ch=${record.channelCount}")
    }
        ...
        for ((rate, chMask) in combos) {
            try {
                ...
                if (rec.state == AudioRecord.STATE_INITIALIZED) {
                    Log.i(TAG, "AudioRecord ok: $rate Hz mask=$chMask")
                    return rec
                }
                rec.release()
            } catch (e: Exception) {
                Log.w(TAG, "combo $rate/$chMask failed: ${e.message}")
            }
        }
        return null
    }
    ...
    private fun createChannel() {
        ...
        nm.createNotificationChannel(
            NotificationChannel(CHANNEL, "直播翻译", NotificationManager.IMPORTANCE_LOW)
        )
    }
