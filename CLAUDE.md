...
当前版本：`2.2.0` / versionCode `32`。

## 不可破坏的产品边界

1. 主导航固定为 **同传 / 视频 / 历史 / 设置**。
2. 同传和视频分别维护场景库、默认场景、`TranslationPlan` 草稿、命名方案与本场上下文，禁止跨模式复用或覆盖。
3. **场景库是场景提示词的唯一运行时真源**：`DefaultSceneCatalog` 只负责首次初始化与恢复模板；所有场景条目都允许用户编辑。
4. **方案只引用场景 ID 和方案级额外提示词**；不要恢复“自定义场景说明”这条重复路径。**语言方向不绑进方案**：语言是每模式独立、随时可调的运行时选择（存在草稿里、由主页语言胶囊设置、启动时进快照），套用已保存方案只改场景与额外提示词、保留当前语言；方案编辑器和方案卡都不再承载语言方向。
5. **本场临时上下文只在同传/视频主页**，不写入方案库；视频页额外保留视频链接与 AI 解析。
6. 术语库已经从当前 UI 和提示词链路移除，不要恢复入口、词条或演示数据。
7. 启动翻译前冻结完整 Prompt、语言、场景 ID 与场景名称。权限回调、重连和前台服务只能继续该快照，不能重新读取当前配置。
8. 方案库的“编辑”与“应用”是两个动作：编辑已有方案必须按 ID 原位更新；取消编辑不能污染当前草稿。
