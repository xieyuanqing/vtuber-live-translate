...
5. **本场临时上下文只在同传/视频主页**，不写入场景库；视频页额外保留视频链接与 AI 背景分析。AI 结果先进预览，用户确认后才写入输入框，不直接覆盖手写内容。
...
| `AiTextClient.kt` / `ContentContextAnalyzer.kt` | 独立背景分析 AI，不参与实时连接；地址会去掉多写的 /v1 版本段 |
| `VideoMetadataClient.kt` / `WebPageReaderClient.kt` | 三平台专用元数据抓取，以及未知公网播放页的 Jina Reader 受限文本提取 |
...
个人自用工具，优先直白实现，不为想象中的需求提前抽象。这几条是踩过的坑（曾残留几百行死代码）：

- **删功能要删干净**：移除一个功能时，连它的数据模型、Store、Prompt 段落和测试一起删，并用全局搜索确认零引用。别只删 UI 入口留下背后整条链（术语库就曾只删 UI，留下 store / model / prompt / 测试）。
- **不写投机抽象**：不为「以后可能要」加参数、回调、接口或密封类。一个钩子/参数若没有真实调用方传入，就不要它——只被测试用到的抽象等于死代码（导航层的 accessGate / 审计钩子即是）。
- **防御只对应真实故障**：容错、重试、多级 fallback 只针对实际观察到的问题（连接轮换、机型音频回退、权限竞态等已列在下方「容易踩的坑」，改动前先看，别误删）。不要为当前 minSdk / 数据流下不可能发生的情况堆防御（如 `minSdk 29` 下写 `SDK_INT >= 26` 分支，或把 `2.4.0` 硬凑成 versionCode）。
- **改前先确认在用**：动一段代码前先查它是否真被调用；无引用的函数、字段、重载直接删，不要「顺手保留」。
