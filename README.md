...
- **AI 内容分析**：在对应业务页整理现场资料或视频元数据，生成本场上下文和候选术语；候选术语仅在点击“应用”后写入术语库
- **实时字幕**：App 内字幕流，以及可暂停、可切换普通/紧凑形态的系统悬浮字幕
- **结构化历史**：仅在 App 私有目录保存模式、语言方向、时长、原文和译文，支持详情查看与 Markdown 复制，不自动向公共 Downloads 写入副本
...
开始翻译时会生成不可变会话快照。权限回调、重连和后台服务不会重新读取正在编辑的方案，因此同传与视频的语言、场景、提示词和本场上下文不会互相覆盖。本场资料会随 Activity 重建保存，并在启动快照成功交给服务后消费。
...
- 悬浮字幕需要系统悬浮窗权限
- Android 13 及以上可允许通知以查看前台服务通知；拒绝通知权限不会阻止录音和翻译
...
- 强化 Android 11–13 麦克风前台服务类型、权限回调和会话生命周期
- 方案库与术语库逐项容错，损坏数据在修复写入前保留原文备份
- AI 候选术语延迟到用户应用方案时持久化，关闭面板不会留下孤立术语
- 自动历史仅保存在 App 私有目录，避免删除 App 内历史后仍有公共副本
