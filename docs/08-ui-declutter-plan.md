...
> 落地：设置两组（翻译 / 系统），六行入口 ID 不变。

### B6 设置二级页：翻译服务 / 背景分析 AI（补做）

B0–B5 漏掉了设置二级页。两页干的是同一件事（配一个 AI 服务：Key + 地址 + 模型 + 自检），
却长成两种样子：AI 页顶部彩色 header 卡与 Toolbar 页名双头部（B0 的问题）、两个分节标签、
Gemini 分支同屏 9 个同权重按钮；翻译页 5 个按钮 3 种样式挤在一张卡里，折叠开关和动作按钮同形。

目标骨架（两页一致）：一句作用说明 → 一张连接卡（输入 → 唯一实底主按钮「测试…」→ 状态行）
→ 可选项折叠行 → 底部凭据存储说明。按钮权重按 R5 分三层：实底 Tonal 只给「测试」；
`FieldActionButton`（outlined）给粘贴 Key / 复用 Key / 拉取模型列表；`FieldLinkButton`（文字）
给申请 Key / 恢复默认。折叠行用 `DisclosureRow`（整行 + 行尾 chevron），与动作按钮形态区分。

硬约束：**「拉取模型列表」必须保持全宽可见、紧贴模型输入框**——不能指望用户手输模型 ID，
`helper_second_ai_model` 也要把拉取写成主路径。

> 落地：两页统一骨架；删彩色头卡与分节标签；申请 Key 入口并入教程对话框；
> 新增 `rowSecondAiKeyActions` 让 Key 相关动作随 Key 字段一起显隐（Zen 模式整组收起）。
