...
> 历史重构记录：2026-09-07 已移除好友邀请网关、绑定、设备签名和凭据模式字段；下文相关内容仅用于追溯当时设计，不代表当前实现。当前结构以源码与 CLAUDE.md 为准。

> **锁定方式**：以本文件所在提交中的内容为施工真源；修订起点为 `90bf642`。不要按修订起点读取旧版文档。
>
> **状态**：**已完成**（2026-07-18）。步骤 1–6 均已独立提交；`MainActivity` 仅保留生命周期、launcher、controller 接线、300 ms 状态分发与窄 host。CI 与真机冒烟均通过。本文保留为结构边界与验收矩阵的参考，不再按“待执行”施工。
>
> **落地提交（main）**：
>
> 1. `8332e8b` 抽出历史页面控制器  
...
### 10.2 完成记录（2026-07-18）

| 项 | 结果 |
|---|---|
| 步骤 1–6 独立提交 | 已完成，见文首提交列表 |
| `MainActivity` 职责 | 约 269 行；仅生命周期 / launcher / wiring / 状态分发 / 窄 host |
| 页面归属 | `ui/HistoryController`、`SceneLibraryController`、`SettingsController`、`SessionCoordinator`、`ModeHomePages`、`SessionContextController`、`MainNavigator`、`UiRuntimeStatus` |
| 布局 / 版本 / 管线 | 未改 `res/layout/`、未升 version、未改实时翻译管线 |
| CI | `main` 上 `android-debug.yml` 对应 `bca032c` 成功（assemble + 单测/Lint 按工作流） |
| 真机冒烟 | 通过：四 tab、场景库、历史详情、设置子页、旋转/重建恢复、权限拒绝、同传启动、视频投屏取消、overlay 设置入口 |
| 开发日志 | 见 `docs/04-dev-log.md` 同日条目 |

后续若再动 UI 壳层，仍须遵守本文第 2–6 节不变量与第 8 节 SaaS 扩展缝；不要把业务逻辑重新堆回 `MainActivity`。第 8 节 SaaS 扩展缝仅定义边界，默认不实现。
