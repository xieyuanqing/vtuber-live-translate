...
- 修复断线重连撞上 505 秒轮换时重复建连接、停止后遗留连接的问题。重连取消轮换定时器，建连接统一关闭旧 socket，并准确显示重连 / 轮换状态；原有队列与异常断线音频回填保持。
- 启动读取的配置和临时补充用于本场快照；运行中改设置不改变重连使用的配置。等待元数据时快速切视频不再写回旧视频资料，暂停或静音时输入音量归零。
- 弹窗加宽至 360px，语言并排，状态独立成行，错误消息完整换行；加入「取消启动」「应用当前设置并重新开始」和页面未连接时的刷新入口。临时补充立即传到页面内存，避免关闭弹窗导致防抖输入丢失；快速连续改配置串行保存。
- 安装说明补充固定目录使用方式：安装文件独立于 Git 工作区，避免切到没有扩展的分支后插件文件不可用。

**版本**：扩展 0.1.1；Android 仍为本分支原有 2.4.1 / 36。

**验证**：扩展原有自检、17 个 JavaScript 文件语法检查、`git diff --check` 全部通过；10 项会话 / 音频挂载 / 连接竞态回归检查全部通过。独立无界面 Chromium 离线检查弹窗加载、临时补充、切语言后应用重开、长错误换行和页面恢复入口，无脚本异常；测试使用模拟消息，不连接真实翻译服务。Android 单元测试、Lint、assembleDebug 全部通过。未操作用户正在观看直播的浏览器，未做真实直播长跑。

---

## 2026-09-06 · 弹窗「本场临时补充」

- 看直播时发现不少场次标题简介提供不了多少背景，于是在弹窗加了「本场临时补充」输入框：当场补人名对照、当前游戏名这类信息，开始翻译时和长期背景、标题简介一起冻进同一条不可信围栏。
- **刻意做成易失的**：只存内容脚本内存（不进 `chrome.storage`），换视频或刷新页面即清空——它本来就是「这场的事」，持久需求该走设置里的长期背景。运行中改动遵循冻结快照原则，提示「停止再开始后生效」；点「开始翻译」前会先把未落库的输入补发给内容脚本，避免快照冻不住刚打完的字。
- 实现走一条新消息 `SET_TEMP_CONTEXT`（弹窗 → 内容脚本，输入防抖 400ms），`Prompt.build` 新增 `tempContext` 参数拼进围栏，状态快照带上 `tempContext` / `usedTemp` 让弹窗回显。自检补了围栏顺序用例。
...
**验证**：`node extension/tools/selftest.js` 全部通过——重采样（48k/44.1k/16k 各批大小下不漂移、跨批相位连续、立体声反相混音归零）、字幕稳定器（重叠合并、单字重叠按巧合处理、切句、整句复读丢弃、单碎片多句全保留、flush 残留）、提示词围栏顺序与简介截断、设置范围收敛、manifest 引用完整性。所有 JS 过 `node --check`。

**浏览器实测通过**（2026-09-05，鈴原るる NieR:Automata 直播，日→中）：**浏览器可以直连 Gemini Live Translate WebSocket**，这是立项时唯一的生死判定项，现已确认成立——`?key=` 查询参数鉴权 + 安卓版那套 setup 结构在扩展的内容脚本里原样可用。音频旁路挂载、自动开始、字幕注入播放器、「确认行 + 当前行」双行渲染均正常。
