...
真实设备修正后采用展开/侧边收起两态，不改变 WindowManager 类型、权限、拖拽、暂停和翻译会话。收起只改变窗口展示，不得隐式暂停或停止翻译。
...
- 保留独立 pause，并提供收起到最近屏幕侧边的按钮；不增加 fullscreen。
- Web blur 用半透明底色、elevation 和必要文字阴影近似。
- 根窗保持 `WRAP_CONTENT`，展开宽度约屏幕 88%、最大 360dp；装饰条必须由根 View 自绘，禁止在 `WRAP_CONTENT` 根窗内放 `MATCH_PARENT` 高度装饰子项，以免部分 ROM 将半透明窗口测成整屏。

### 12.2 侧边收起态

- 点击展开态右上角收起按钮后，窗口吸附到当前位置最近的屏幕边缘，缩为约 44×60dp 的蓝色小胶囊。
- 胶囊只显示“译”和朝向屏幕内侧的展开箭头；点击后恢复收起前的尺寸与位置。
- 收起和展开都不得发送 pause/stop Service Intent，不重建翻译会话；字幕仍在后台持续更新。
- 胶囊纵向位置和恢复后的展开窗口都必须约束在当前屏幕范围内。
